# ASL Live Translation — Raspberry Pi 5 version
# Uses TFLite (via tensorflow) + cv2 GStreamer pipeline (Camera Module 3)
...
# Pygame needs dummy audio driver on Pi
os.environ['SDL_AUDIODRIVER'] = 'dummy'
# Ensure tkinter finds the display when running over SSH/VNC
...
class LogoPlayer:
    ...
    def set_size(self, new_size):
        ...
        self._scaled_cache = {}   # invalidate cache at new size
...
def main():
    ...
    # Pi Camera Module 3 via GStreamer + libcamera pipeline
    cap = cv2.VideoCapture(
        "libcamerasrc ! videoconvert ! video/x-raw,format=BGR ! appsink",
        cv2.CAP_GSTREAMER
    )
    if not cap.isOpened():
        # Fallback to direct index if GStreamer fails
